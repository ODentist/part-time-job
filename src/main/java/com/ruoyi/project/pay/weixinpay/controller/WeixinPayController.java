...
import com.ruoyi.project.pay.common.constants.Constants;
import com.ruoyi.project.pay.common.model.Product;
import com.ruoyi.project.pay.common.utils.DateUtil;
import com.ruoyi.project.pay.event.WeixinEvent;
import com.ruoyi.project.pay.weixinpay.service.IWeixinPayService;
import com.ruoyi.project.pay.weixinpay.util.ConfigUtil;
import com.ruoyi.project.pay.weixinpay.util.HttpUtil;
import com.ruoyi.project.pay.weixinpay.util.PayCommonUtil;
import com.ruoyi.project.pay.weixinpay.util.XMLUtil;
...
public class WeixinPayController {
	...
	@RequestMapping(value="qcPay1",method=RequestMethod.GET)
    public String  qcPay1(Product product, ModelMap map) {
		logger.info("二维码支付(模式一)");
		Random random = new Random();

		String orderNo = DateUtil.date2Str(new Date(), "yyyyMMddhhMMss") + String.valueOf(random.nextInt(90000));
		product.setOutTradeNo(orderNo);
		product.setProductId(String.valueOf(random.nextInt(90000)) + ShiroUtils.getUserId());

		weixinPayService.weixinPay1(product);
		String img= "../qrcode/"+product.getProductId()+".png";

		map.addAttribute("orderNo", orderNo);
		map.addAttribute("totalFee", product.getTotalFee());
		map.addAttribute("img", img);
		context.publishEvent(product);
		return "weixinpay/qcpay";
    }


	@ApiOperation(value="二维码支付(模式二)下单并生成二维码")
	@RequestMapping(value="qcPay2",method=RequestMethod.GET)
    public String  qcPay2(Product product,ModelMap map) {
		...
		String orderNo = DateUtil.date2Str(new Date(), "yyyyMMddhhMMss") + String.valueOf(random.nextInt(90000));
		product.setOutTradeNo(orderNo);
		product.setSpbillCreateIp("127.0.0.1");
	...
	public void weixin_notify(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (PayCommonUtil.isTenpaySign("UTF-8", packageParams, key)) {
			...
			if ("SUCCESS".equals((String) packageParams.get("result_code"))) {
				...
				context.publishEvent(new WeixinEvent(orderNo, "SUCCESS", (String) packageParams.get("total_fee")));
			} else {
